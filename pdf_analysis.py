...
def fit_function(pdfs, *alphas): #where I can find alpha??
    '''
...
def get_pdfs(toccsl_data, sm_data=None, sm_start_frame=None, rec_frame=5, lim=4000, lim_con=2, sampled_sm=False, perc=0.5):
    ...
    x1, y1 = get_pdf(data_rho1, lim=lim)
    xdata, ydata = get_pdf(data_rhodata, lim=lim)
...
def get_results(toccsl_data, sm_data=None, sm_start_frame=None, rec_frame=5, lim=4000, lim_con=2, iterations=100, perc=0.5, save_path=os.getcwd() + '\\data\\TOCCSL_results_n{}.png'):
    '''
    ...
    ### Generate PDFs
    start = time.time()
    pdfs, pdf_data = get_pdfs(toccsl_data=toccsl_data, sm_data=sm_data, sm_start_frame=sm_start_frame, rec_frame=rec_frame, lim=lim, lim_con=lim_con)
    ...
    for i in tqdm(range(iterations), desc='bootstrap iterations'):
        ### Generates PDFs
        pdfs_s, pdf_data_s = get_pdfs(toccsl_data=toccsl_data, sm_data=sm_data, sm_start_frame=sm_start_frame, rec_frame=rec_frame, lim=lim, lim_con=lim_con, sampled_sm=True, perc=perc)
        ### Fit making sure no alpha_i is samller than zero
