...
def brightness_final(data_sm, data_loc_sm, single_molecule_size, single_molecule_mean, sm_start_frame, sm_final_frame):
    cols=sns.color_palette("pastel", n_colors=len(data_loc_sm)) #ns.color_palette("Spectral", as_cmap=True)
...
#data_loc_sm[0]['mass']
   ...
   #     ax.hist(overall[j], bins=bins[j], density=True,  color=cols[j], alpha=0.4 , label=f'sm filename {j}, mean brightness= {int(single_molecule_mean[j])}')
       # ax.plot(bins[j], norm.pdf(bins[j], *param[j]), '--' , color=cols[j], label=f'PDF (Gauss) filename {j}, maximum brightness= {int(x_at_max_pdf[j])}')#+ str(norm.pdf(param[0], param[0], param[1])))
        ...
        x_kde=np.linspace(0, lim, int(lim/4))
        y_kde= gaussian_kde(overall[j]).pdf(x_kde)
        ax[0].plot(x_kde,y_kde, color=cols[j])#, label=f'PDF (Gauss_kde) filename {j}, maximum brightness= {int(x_at_max_pdf[j])}')
   ...
   #     x_at_max_pdf.append(bins[j][max_index[j]])
    ...
    # Get corresponding x value
     #   x_at_max_pdf.append(bins[j][max_index[j]])
        ...
        lim=2.5
        x_kde=np.linspace(0, lim, 100)
        y_kde= gaussian_kde(overall[j]).pdf(x_kde)
        ax[1].plot(x_kde,y_kde, color=cols[j])#, label=f'PDF (Gauss_kde) filename {j}, maximum brightness= {int(x_at_max_pdf[j])}')
 ...
 #   ax[1].legend(loc='upper center', bbox_to_anchor=(0.5, -0.25))
   # ax[1].legend(loc='center left', bbox_to_anchor=(1, 0.5))
    ...
    for j in range(0,len(data_loc_sm)):
        ...
        for i in range(0,len(data_loc_sm[j])):
            ...
            ax[2].plot(data_loc_sm[j][i]['mass'], data_loc_sm[j][i]['size'], '.', color=cols[j], alpha=0.2)
  ...
  #  cols=sns.color_palette("viridis", n_colors=len(data_loc_sm[0]))
    ...
    #mean.append(np.mean(frame_overall[i]))
        ...
        ax[3].plot(frame_all_file[file], color=cols[file], label=f'filename {file}')
    ...
    return fig
#create the pd file with the normalized data
#data_file=data_loc_sm[3][0]
#data={'x':  data_file['x'], 'y':  data_file['y'], 'signal':  data_file['signal'],'bg':  data_file['bg'], 'mass':  data_file['mass'], 'size':  data_file['size'], 'frame':  data_file['frame']}#, 'delay '+str(delay)+' ms': np.round(np.linspace(0,len(bin),len(bin)),0)*delay}#, 'frame': norm}
#data = pd.DataFrame(data)

#data.to_csv(f'{data_sm}/'+'20250512_pmhc_2ng_slb1_290min_0000'+'.csv')

#data_files_sm_filename=['20250512_pmhc_2ng_slb1_20min_000','20250512_pmhc_2ng_slb1_160min_000', '20250512_pmhc_2ng_slb1_230min_000', '20250512_pmhc_2ng_slb1_290min_000']''''